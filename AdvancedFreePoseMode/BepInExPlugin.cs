...
{
    [BepInPlugin("bugerry.AdvancedFreePoseMode", "Advanced Free Pose Mode", "1.0.0")]
    public partial class BepInExPlugin : BaseUnityPlugin
    {
        ...
        public static ConfigEntry<float> horizontal_sensitivity;
        public static ConfigEntry<float> vertical_sensitivity;
        public static ConfigEntry<float> rotation_sensitivity;
        ...
        private void Awake()
        {
            ...
            horizontal_sensitivity = Config.Bind("Sensitivity", "Horizontal", 1f, "The sensitivity of the object placement horizontal");
            vertical_sensitivity = Config.Bind("Sensitivity", "Vertical", 20f, "The sensitivity of the object placement vertical");
            rotation_sensitivity = Config.Bind("Sensitivity", "Rotation", 20f, "The sensitivity of the object placement rotation");
            ...
            keepPose = Config.Bind("Free Pose Mode", "Keep Pose", true, "Keep pose after Free Pose Mode");
        ...
        public static class UIFreePose_AddCharacter_Patch
        {
            public static void Postfix(Transform character, UIFreePose __instance)
            {
                ...
                if (character.TryGetComponent(out Animator anim)) anim.enabled = !keepPose.Value;
            }
        }
        ...
        public static class UIFreePose_Close_Patch
        {
            public static void Prefix(UIFreePose __instance)
            {
                ...
                {
                    if (character.TryGetComponent(out ThirdPersonCharacter tcp)) tcp.enabled = true;
                }
            ...
            public static bool Prefix(MoveObject __instance)
			{
                ...
                if (__instance.canGo && Global.code.uiFreePose.selectedCharacter && context.lastPositions.TryGetValue(__instance, out lastPos))
                {
                    ...
                    if (__instance.move)
                    {
                        ...
                        __instance.mover.transform.position += a * __instance.speed_Move * horizontal_sensitivity.Value * Time.deltaTime;
                    }
                    else if (__instance.moveY)
                    {
                        __instance.mover.transform.position += new Vector3(0f, __instance.deltaY * __instance.speed_MoveY * Time.deltaTime * vertical_sensitivity.Value, 0f);
                    }
                    else if (__instance.rotate)
                    {
                        __instance.mover.transform.eulerAngles += new Vector3(0f, -__instance.deltaX * __instance.speed_Rotate * Time.deltaTime * rotation_sensitivity.Value, 0f);
                    }
                    else if (__instance.rotatelighting)
                    {
                        component.characterLightGroup.transform.eulerAngles += new Vector3(0f, -__instance.deltaX * __instance.speed_Rotate * Time.deltaTime * rotation_sensitivity.Value, 0f);
                    }
